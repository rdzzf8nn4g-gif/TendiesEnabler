...
- (void)powerStateChanged {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (g_enabled && g_isVideoMode && g_isScreenOn) {
            [self onWakeUp:nil]; // 复用校验逻辑秒关/秒开
        }
    });
}

- (void)onWakeUp:(NSNotification *)note {
...
- (void)onAOD:(NSNotification *)note {
    if (!g_enabled || !g_isVideoMode) return;
    [self.lockVideoView pauseVideo];
    [self.homeVideoView pauseVideo];
}
...
- (void)reloadWallpaperViews {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (IsSingleVideoMode()) {
            ...
            if (g_isScreenOn) [self onWakeUp:nil];
        } else {
            if (hasLock) {
...
- (void)onWakeUp:(NSNotification *)note {
    ...
    self.isUnlocking = NO;
    BOOL animated = note ? [note.userInfo[@"animated"] boolValue] : NO;
    [CATransaction begin]; [self transitionToState:g_isUnlocked ? @"Unlock" : @"Locked" animated:animated]; [CATransaction commit]; [CATransaction flush];
}

- (void)onAOD:(NSNotification *)note {
    if (!g_enabled || !self.bgView) return;
    self.isUnlocking = NO;
    BOOL animated = note ? (note.userInfo[@"animated"] ? [note.userInfo[@"animated"] boolValue] : YES) : YES;
...
- (void)viewDidLoad {
    ...
    if (g_enabled) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(zone_screenSleep) name:@"ZoneEngineAOD" object:nil];
...
- (void)dealloc {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"ZoneEngineAOD" object:nil];
...
- (void)setBacklightState:(long long)state source:(long long)source {
    ...
    if (g_enabled) {
        if (state == 0) {
            ...
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineSleep" object:nil userInfo:@{@"animated": @(NO)}];
            });
        } else if (state == 1) {
            g_isScreenOn = YES;
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineAOD" object:nil userInfo:@{@"animated": @(NO)}];
            });
        } else {
            g_isScreenOn = YES;
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineWake" object:nil userInfo:@{@"animated": @(NO)}];
            });
        }
    }
}
- (void)setBacklightState:(long long)state source:(long long)source animated:(BOOL)animated completion:(id)completion {
    ...
    if (g_enabled) {
        if (state == 0) {
            ...
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineSleep" object:nil userInfo:@{@"animated": @(animated)}];
            });
        } else if (state == 1) {
            g_isScreenOn = YES;
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineAOD" object:nil userInfo:@{@"animated": @(animated)}];
            });
        } else {
            g_isScreenOn = YES;
            g_lastTickProgress = -1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineWake" object:nil userInfo:@{@"animated": @(animated)}];
            });
        }
    }
}
...
- (void)updateWakeEffectsForWake:(BOOL)wake animated:(BOOL)animated completion:(id)completion {
    %orig;
    if (g_enabled && !g_isVideoMode) {
        NSString *targetState = wake ? (g_isUnlocked ? @"Unlock" : @"Locked") : @"Sleep";
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ZoneEngineStateChange" 
                                                                object:nil 
                                                              userInfo:@{@"state": targetState, @"animated": @(animated)}];
        });
    }
}
%end
